...
"")
    ...
    def send(self):
        ...
        print(dataString)
    def get_single_tip_point(self):

        self.px =float(input("insert x:"))
        self.py =float(input("insert y:"))
        self.pz =float(input("insert z:"))
        self.plot_data()
        self.px = float(input("insert x:"))
        self.py = float(input("insert y:"))
        self.pz = float(input("insert z:"))
...
class linear_move:
    ...
    def move(self):
        interval=50
        roll=pitch=yaw=1
        x_vec = numpy.linspace(self.px,self.px_new,interval)
        y_vec = numpy.linspace(self.py, self.py_new, interval)
        z_vec = numpy.linspace(self.pz, self.pz_new, interval)
        for i in range(interval):
            q1, q2, q3, q4, q5, q6 = get_angles(x_vec[i], y_vec[i], z_vec[i], roll, pitch, yaw)
            X,Y,Z = forward_kin(q1,q2,q3,q4,q5,q6)
    ...
    def get_single_tip_point(self):
        ...
        self.px_new =float(input("insert x:"))
        self.py_new =float(input("insert y:"))
        self.pz_new =float(input("insert z:"))
...
def main():
        ...
        roy=linear_move()
        while(1):
            roy.get_single_tip_point()
            roy.parbolic_move()


        #ui = Ui_ArmControl()
        #ui.setupUi(MainWindow)
        #ui.get_single_tip_point()
        #MainWindow.show()
        plt.show()
        sys.exit(app.exec_())
